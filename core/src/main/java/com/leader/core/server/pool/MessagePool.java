...
public class MessagePool {
	...
	public void init() {
		...
		// Protocol protocol = handlerClass.getAnnotation(Protocol.class);
		// if (protocol != null) {
		// String handlerName = TextKit.replace(handlerClass.getSimpleName(),
		// "Handler", "");
		// String className = packagepath + "." + protocol.value() + "$" +
		// handlerName + "Message";
		// Class<?> mc = messageType.getClass();
		// try {
		...
		// Class<?> c = Class.forName(className);
		// Method method = c.getMethod("parser");
		// Parser<? extends Message> parser = (Parser<? extends Message>)
